...
class Cylinder_side(object):
    def __init__(self, vert_shader, frag_shader, radius, height, numSide):
        #  a Cube Made of Two Triangle Strips Using Primitive Restart
        self.sectorCount = numSide
    ...
    def getSideVertices(self, radius, height):
        ...
        for x in range(0, self.sectorCount + 1):
            ...
            unitCircleVertices.append(radius * math.cos(angle + sectorAngle))
            unitCircleVertices.append(height)
            unitCircleVertices.append(radius * math.sin(angle + sectorAngle))
            unitCircleVertices.append(radius * math.cos(angle + sectorAngle) * 0.5)
            unitCircleVertices.append(-height * 0.5)
            unitCircleVertices.append(radius * math.sin(angle + sectorAngle) * 0.5)
